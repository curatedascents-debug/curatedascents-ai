...
export default function PackagesTable({ rates, onSelect }: PackagesTableProps) {
  return (
    <div className="bg-slate-800 rounded-lg overflow-hidden">
      <table className="w-full">
        <thead className="bg-slate-700">
          <tr>
            ...
            <th className="text-left p-4 font-medium">Group Size</th>
        ...
        <tbody>
          {rates.map((rate, i) => (
            ...
            >
              ...
              <td className="p-4 text-slate-300 capitalize">{rate.packageType?.replace("_", " ") || "-"}</td>
              <td className="p-4 text-slate-300">
                {rate.durationDays ? `${rate.durationDays}D${rate.durationNights ? `/${rate.durationNights}N` : ""}` : "-"}
              </td>
              <td className="p-4 text-slate-300">{rate.difficulty || "-"}</td>
              <td className="p-4 text-slate-300">
                {rate.groupSizeMin || rate.groupSizeMax
                  ? `${rate.groupSizeMin || "1"}-${rate.groupSizeMax || "?"}`
                  : "-"}
              </td>
      ...
      {rates.length === 0 && <div className="p-8 text-center text-slate-400">No package rates found.</div>}
    </div>
  );
}
